...
This document describes my design, database models, API routes, WebSocket event handlers, security controls, and integration implemented for the **Real-Time Chat & Message Management System** in CodeMesh. It explains my code logic and architectural decisions behind the implementation.

---

## 1. Overview of Phase 4

My main objective for Phase 4 was to transition CodeMesh from a static workspace registry to an interactive, real-time collaboration environment. This phase covers:
...
```
* **Security Rationale**: A user could construct random UUIDs and scan the `/messages` route. By checking the user's membership in the parent `workspaceId` prior to fetching, I guarantee data privacy.
* **Projections**: My query uses `include.sender.select` to return safe details (e.g., name, avatar) but entirely excludes sensitive fields like `passwordHash`.

---

## 4. WebSocket Manager ([socket.js](file:///d:/Projects/CodeMesh/backend/src/lib/socket.js))

Instead of packing Socket.IO inside `index.js`, I implemented a modular helper in `socket.js`.
...
When a client connects and requests to join a channel room, I re-verify workspace membership:
```javascript
...
When a message is received from a client, I insert it into PostgreSQL and broadcast the newly created message:
```javascript
...
WebSockets cannot run directly on a raw Express application object. I integrated Node.js's native `http` module:
